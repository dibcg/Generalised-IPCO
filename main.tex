...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Suppose $G$ contains $(a,b)$-quasi-isometric embedding of $C_k$.  First observe that the $C_k$ must be mapped to a subgraph of $G$ whose girth depends on $k,a,b$. Define $k'$ to be much lasrger than this girth. 
		
		\item Consider a vertex $v$ of $H$ and a ball $B_v$ around $v$ whose radius is a function of $k,a,b$. 
		
		\item Since $k'$ is much larger, the graph $H'$ induced by $B_v$ is a tree. 
		
		\item Let us look at the vertices of $G$ which are mapped to close to the vertices of $B_v$. Let us call this set $S$.
		
		\item Define $k'$ in such a way that we can infer the metric $(S,G)$ to be a tree metric and hence cannot embed a cycle metric with small distortion.
